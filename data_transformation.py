...
from pyspark.sql.functions import when, col, regexp_extract, split
...
# Script generated for node parquet_input
dyf = glueContext.create_dynamic_frame.from_options(
    format_options={},
    connection_type="s3",
    format="parquet",
    connection_options={
        "paths": ["s3://de-job-project/de-project/data/raw_data_parquet/"],
        "recurse": True,
    },
...
# Check the Seniority in the tittle
...
cloud_columns = [when(col("job_description").rlike(f"(?i){keyword}"), True).otherwise(False).alias(column_name)
                 for keyword, column_name in keyword_tools]

df_final = df.select("source_date",
                     "job_title",
                     "company_name",
                     "job_location",
                     "company_size",
...
# Script generated for node parquet_output
s3output = glueContext.getSink(
    path="s3://de-job-project/de-project/data/parquet_data_output/",
    connection_type="s3",
    updateBehavior="UPDATE_IN_DATABASE",
    partitionKeys=["source_date"],
    compression="snappy",
    enableUpdateCatalog=True,
    transformation_ctx="s3output",
)
s3output.setCatalogInfo(
    catalogDatabase="de-project-database", catalogTableName="transformed_data"
)
